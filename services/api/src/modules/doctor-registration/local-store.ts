...
import { assertRailwayVolume, railwayScanner } from './railway-volume.js';
...
// Private encrypted disk adapter; mode-specific setup enforces local or mounted-volume policy.
export class LocalCredentialStore implements PrivateCredentialStore {
  readonly available = true;
  constructor(private root: string, private key: Buffer, private scan: ScanFile, private assertMount: () => void = () => {}) {
...
export function configuredRegistration(env: Environment): RegistrationDependencies {
  ...
  if(env.DOCTOR_CREDENTIAL_STORAGE==='railway-volume') {
    ...
    return {store:new LocalCredentialStore(env.DOCTOR_CREDENTIAL_ROOT,Buffer.from(env.DOCTOR_CREDENTIAL_KEY,'base64'),railwayScanner.scan,()=>assertRailwayVolume(env)),
      requiredKinds:env.DOCTOR_REQUIRED_CREDENTIALS.split(',') as CredentialKind[]};
  }
  if(!['test','development'].includes(env.APP_ENV)||env.NODE_ENV==='production')throw failure();
