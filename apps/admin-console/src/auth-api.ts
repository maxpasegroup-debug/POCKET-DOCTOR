...
export function adminOtpPreview(challenge: AdminChallenge, localDevelopment: boolean): string | undefined {
  if (!challenge.developmentCode || !/^\d{6}$/.test(challenge.developmentCode)) return undefined;
  if (challenge.delivery === 'testing') return `Staging test code: ${challenge.developmentCode}. No SMS was sent. An authenticator code is still required.`;
